...
| 41 | WorldSync/Resource | `Core/Registration/SteamP2PFriendsPlugin.PatchRegistrationLegacyDiagnostics.cs` → `Adapters/Resource/Patches/ResourceManagerWorldSyncDiagnosticPatch.cs` | `ResourceManager.onRegionUpdated()`；`ReceiveResources()` Prefix+Postfix | Prefix / `OnRegionUpdated_Prefix`、`ReceiveResources_Prefix`；Postfix / `ReceiveResources_Postfix` | com.yu80rice.steamp2pfriends / default | 每个 identity target 独立核验；`SendResources_Write(NetPakWriter,byte,byte)` 由 ResourceRegionSync 唯一拥有；依赖 Resource Manager lifecycle | S+H+R-Pending |
