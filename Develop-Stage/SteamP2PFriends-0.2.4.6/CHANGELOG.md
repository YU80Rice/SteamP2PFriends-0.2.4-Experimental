...
## v0.2.4.4-experimental - 2026-08-25 (PASS - 已冻结归档)

- M4 运行验收：基于房主（234906）、虚拟机客机（234908）、用户客机（235308）三端 UMM 诊断包完成全量证据审计，100% PASS。
- M4：新增 `ZombieSnapshotAdapter` 与 `ZombieSnapshotReplicationLedger`，实现逐观察者僵尸快照与外观复制管理。
- M4：显式绑定 `ReliableEnqueueBaseline`，支持会话代、连接代、区域代一致性校验与异常回滚。
- M4：增量状态维护单调递增 `DeltaSequence`，杜绝过期/乱序状态干扰。
- M4：新增 10 项全量状态机与隔离测试；自动化测试套件扩展至 `114/114 PASS`。

## v0.2.4.3-experimental - 2026-08-24 (PASS - 已冻结归档)

- M3 运行验收：基于房主（005454）、虚拟机客机（005449）、用户客机（005459）三端 UMM 诊断包完成全量证据审计，12 项准则全部 PASS。
- M3：新增 `ZombieRegionLifecycleAdapter`，以原生 `PlayerCountInRegion/regionsWithPlayers` 作为功能需求首选事实源。
- M3：远端观察者首次需求复用原生 `generateZombies`，提交 `RegionGeneration` 并保持原生发送链路，不新增 RPC。
- M3：最后观察者离区使用有界滞回释放；会话代、区域对象身份和区域代不匹配时拒绝销毁并输出隔离诊断。
- M3：P0-D/P0-E 旧钩子降为兼容入口，临时 `isNetworked/isZombiesLoaded` 写入具备 Apply/Restore/Rollback 代数校验。
- M3：新增 12 项生命周期账本回归；需求不一致时 quarantine 会阻止破坏性释放；当前完整自动化回归 `104/104 PASS`。
- M3：补齐 native demand 自愈：原生玩家计数为正但 `isNetworked=false` 时执行带区域身份校验的受控 Acquire。

## v0.2.4.2-experimental - 2026-08-21

- M2：新增 `ItemObserverReplicationAdapter`，以 observer、连接代、区域、会话代和权威区域代记录原生 `askItems` 基线。
- M2：显式采用 `ReliableEnqueueBaseline`；Prefix 准备、Postfix 提交、Finalizer 异常回滚，不新增物品 RPC。
- M2：`isItemsLoaded` 降为原生兼容投影；相关性退出、重连或新世界代会触发确定性重发，同代重复基线被抑制。
- M2：地图初始快照与拾取移除、玩家/僵尸掉落增量继续分离；U3DS、单机与房主本地路径保持原生。
- M2：异常或后续 Prefix 跳过原生发送时，仅对仍匹配当前连接 token 的 observer/region 回滚 `isItemsLoaded` 兼容投影；远端断开按 SteamID 精确清理连接代。
- M2：Debug/Release 0 errors / 0 warnings，自动化回归 `92/92 PASS`，独立静态复审 PASS；仍等待当前 Debug 哈希的双端运行验收。

## v0.2.4.1-experimental - 2026-08-21

- M1：新增 `ItemGenerationAuthorityAdapter`，在 `ItemManager.onRegionUpdated(step 5)` 原生区域需求循环内，为 listen-host 远端观察者同步执行原生 `generateItems`，时序早于 `isItemsLoaded=true` 与 `askItems`。
- M1：待审核玩家与已授权玩家采用相同世界观察者资格；审核状态不参与地图自然物品快照生成。
- M1：保留 `AuthoritativeItemGenerationGatePatch` 区域单写者事务；重叠观察者不会二次生成，异常仍由 gate Abort 后允许后续重试。
- M1：`P0-B-3/P0-B-6` listen-host 全图补生成退出编译和启动登记；U3DS 原版全图生成、单机/房主本地懒加载不变。
- M1：新增当前 U3 `Assembly-CSharp.dll` 的真实 IL 形状测试；Debug/Release 编译通过，自动化回归 `81/81 PASS`，等待同哈希双机运行验收。
