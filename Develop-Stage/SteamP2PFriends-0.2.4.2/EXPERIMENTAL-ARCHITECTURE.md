...
## 当前构建阶段：M2 ItemObserverReplicationAdapter

- 版本固定为 `0.2.4.2`，直至 M2 双机运行验收通过；阶段内返修不变更版本号。
- capability 固定为 `ReliableEnqueueBaseline`：原生 `askItems` 正常返回即代表可靠消息已经入队；异常由 Finalizer 回滚账本。
- `ObserverLoadedGeneration` 是逐观察者物品基线真相源；原生 `isItemsLoaded` 只作为兼容投影。
- 连接代变化、相关性退出和会话代变化均会使对应基线失效；同代重复发送被抑制，重入区域必须重发。
- 初始地图物品快照继续复用原生 `askItems`，不新增 RPC；拾取移除、玩家掉落和僵尸掉落增量不被 M2 拦截。
- U3DS、单机和房主本地分支保持原生行为；待审核状态不参与世界观察者资格判断。
