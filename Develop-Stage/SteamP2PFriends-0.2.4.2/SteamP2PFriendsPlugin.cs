...
namespace SteamP2PFriends
{
    ...
    /// </summary>
    [BepInPlugin("com.yu80rice.steamp2pfriends", "SteamP2PFriends", "0.2.4.2")]
    ...
    public class SteamP2PFriendsPlugin : BaseUnityPlugin
    {
        ...
        private void Awake()
        {
            ...
            RoleLogger.Info("[Shared]",
                $"[Startup] version=0.2.4.2 architecture=M2-item-observer-replication p2pEnabled={EnableP2PCoop.Value} " +
        ...
        private static void OnEnemyDisconnectedHandler(SteamPlayer player)
        {
            try
            {
                ...
                if (steamId != 0UL)
                {
                    ...
                    try { Patches.ItemManagerRegionSyncPatch.OnEnemyDisconnected(steamId); }
                    catch (System.Exception itemEx)
                    {
                        RoleLogger.Error("[Shared]",
                            $"OnEnemyDisconnectedHandler(M2 ItemObserver) 异常: {itemEx.Message}");
                    }
                }
