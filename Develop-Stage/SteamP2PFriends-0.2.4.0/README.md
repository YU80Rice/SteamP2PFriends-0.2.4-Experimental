...
`v0.2.4.1` 保留 `v0.2.3.70-beta.2` 的低频 `[P2P-Connection]` 事件，并默认启用只读 `[MultiObserver/M0]` 影子账本。M0 只对照观察者、Item region 和有效 Zombie bound，不更改原生 loaded flag、生成、碰撞、RPC 或审核状态。
...
| 架构阶段 | `M0 Shadow`：只读对照，旧同步 writer 仍是唯一权威 |
| 静态状态 | Release 0 errors / 0 warnings；自动化回归 `67/67 PASS` |
