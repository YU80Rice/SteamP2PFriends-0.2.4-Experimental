...
[![Version](https://img.shields.io/badge/version-0.2.4.1--experimental-blue.svg)](./EXPERIMENTAL-ARCHITECTURE.md)
[![Status](https://img.shields.io/badge/status-M0%20Shadow-orange.svg)](./EXPERIMENTAL-ARCHITECTURE.md)
...
`v0.2.4.1` 保留 `v0.2.3.70-beta.2` 的低频 `[P2P-Connection]` 事件和 M0 影子账本，并进入 M1 Item authority 实验。listen-host 仅在原生 `onRegionUpdated(step 5)` 观察范围内生成地图自然物品；待审核玩家同样属于世界观察者。原生 loaded flag、`askItems` 序列化、拾取移除权威与 U3DS 全图生成保持不变。
...
| AssemblyFileVersion | `0.2.4.1` |
| 架构阶段 | `M1 ItemGenerationAuthorityAdapter`：listen-host 按观察者区域需求生成，旧全图补生成已退出编译 |
| 静态状态 | Debug/Release 0 errors / 0 warnings；自动化回归 `81/81 PASS` |
