...
namespace SteamP2PFriends.Adapters.Item.Patches
{
    ...
    public static class AuthoritativeItemGenerationGatePatch
    {
        ...
        /// <summary>
        /// Read-only M2 bridge. The current gate has one authoritative generation per
        /// session epoch, so the epoch is also the region generation until a later stage
        /// introduces mid-session region rebuilds.
        /// </summary>
        internal static bool TryGetCommittedGeneration(
            byte x,
            byte y,
            out int sessionEpoch,
            out int authoritativeGeneration)
        {
            sessionEpoch = Ledger.Epoch;
            authoritativeGeneration = Ledger.Epoch;
            return Ledger.GetState(x, y) == RegionGenerationState.Committed;
        }
